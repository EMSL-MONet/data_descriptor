...
bgc_long = 
 ...
 #   filter(env_broad_NEW == "temperate") %>% 
  ...
  mutate(name2 = case_match(name,
                            "Bulk_density_g_per_cm3" ~ "Bulk Density",
                            "Total_Carbon_percent" ~ "Total C",
                            "Total_Nitrogen_percent" ~ "Total N",
                            "Total_Sulfur_percent" ~ "Total S",
                            "HClExtOM_TOC_mg_per_kg" ~ "HCl-Ext C",
                            "HClExtOM_TN_mg_per_kg" ~ "HCl-Ext N",
                            "WEOM_TOC_mg_per_kg" ~ "H2O-Ext C",
                            "WEOM_TN_mg_per_kg" ~ "H2O-Ext N",
                            "SO4_S_mg_per_kg" ~"SO4-S",
                            "NH4_N_mg_per_kg" ~ "NH4-N",
                            "NO3_N_mg_per_kg" ~ "NO3-N",
                            "B_mg_per_kg" ~ "B",
                            "Zn_mg_per_kg" ~ "Zn",
                            "Mn_mg_per_kg" ~ "Mn",
                            "Cu_mg_per_kg" ~ "Cu",
  ...
  mutate(data_group = 
           case_when(name2 %in% c("Ca", "Mg", "Na", "K") ~ "cations",
                     name2 %in% c("NH4-N", "NO3-N", "PO4-P", "SO4-S") ~ "nutrients",
                     name2 %in% c("B", "Zn", "Cu", "Fe", "Mn") ~ "micronutrients",
                     name2 %in% c("Total C", "Total N", "Total S", "pH") ~ "bulk chemistry",
                     name2 %in% c("H2O-Ext C", "H2O-Ext N", "HCl-Ext C", "HCl-Ext N", "MBC", "MBN") ~ "SOM"))
...
plot_rainclouds = function(DATA, COUNTS){
  ...
  DATA %>% 
    filter(! (name2 == "Total S" & value > 0.3)) %>% 
    filter(! (name2 == "SO4-S" & value > 200)) %>% 
    filter(! (name2 == "B" & value > 5)) %>% 
    filter(! (name2 == "MBC" & value > 2500)) %>% 
    filter(!is.na(data_group)) %>% 
    ggplot(aes(x = name2, y = value, fill = name2, color = name2))+
...
plot_rainclouds(DATA = bgc_long %>% filter(data_group == "SOM"), COUNTS = bgc_counts %>% filter(data_group == "SOM")) #+scale_y_log10()

```

```{r raincloud_bulk, fig.height=8, fig.width=5}
plot_rainclouds(DATA = bgc_long %>% filter(data_group == "bulk chemistry"), COUNTS = bgc_counts %>% filter(data_group == "bulk chemistry"))

```

```{r raincloud_nutrients, fig.height=8, fig.width=5}
plot_rainclouds(DATA = bgc_long %>% filter(data_group == "nutrients"), COUNTS = bgc_counts %>% filter(data_group == "nutrients"))
