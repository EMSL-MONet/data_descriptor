...
```{r map_ecoregion, fig.height=16, fig.width=12}
...
```{r map_soil_orders, fig.height=10, fig.width=10}
...
plot_rainclouds = function(dat){
  
  dat %>% 
#  filter(data_group == "SOM") %>% 
...
#  geom_point(position = position_jitternudge(width = 0.1, nudge.x = 0.8), size = 0.1, color = "black")+
...
# ggh4x::facet_nested_wrap(~data_group + name2, scales = "free", ncol = 4)+
...
plot_rainclouds(bgc_long %>% filter(data_group == "SOM"))
plot_rainclouds(bgc_long %>% filter(data_group == "bulk chemistry"))  
plot_rainclouds(bgc_long %>% filter(data_group == "nutrients"))
plot_rainclouds(bgc_long %>% filter(data_group == "micronutrients"))
plot_rainclouds(bgc_long %>% filter(data_group == "cations"))
